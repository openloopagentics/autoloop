...
description: Use when running a development loop that should report its status (project, phases, commits) to an Autoloop dashboard. Reports via the autoloop CLI as the loop progresses. Requires AUTOLOOP_API_KEY in the environment and a one-time `autoloop init`.
---

# Autoloop Reporting

Report the loop's status to Autoloop as you work, using the `autoloop` CLI
(`node <repo>/cli/autoloop.mjs`). Reporting is **best-effort observability** — it
must never block or derail the actual development work.

## Prerequisites (set up once)

- **`AUTOLOOP_API_KEY`** must be set in the environment — a per-user key minted via
  `POST /v1/keys` (the user must be a member of the team you report to). The CLI
  never reads the key from a file.
- Run **`autoloop init`** once in the loop's working directory to write `.autoloop.json`:

  ```
  node <repo>/cli/autoloop.mjs init --team <teamId> --project <slug> [--url <apiUrl>]
  ```

  (`<repo>` is wherever `cli/autoloop.mjs` lives. Below, `autoloop` is shorthand for
  `node <repo>/cli/autoloop.mjs`.)
