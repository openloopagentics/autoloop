...
your task/system instructions. The CLI is `node <repo>/cli/daloop.mjs` (referred to
as `daloop` below). It has no dependencies (Node 22+).
...
- Ensure `DALOOP_API_KEY` is in the environment (a per-user key minted via
  `POST /v1/keys`; the user must be a member of the team).
- `daloop init --team <teamId> --project <slug> [--url <apiUrl>]` — writes
  `.daloop.json` in the working directory.
