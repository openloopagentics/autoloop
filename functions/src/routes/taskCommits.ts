...
taskCommitsRouter.put("/:sha", async (req, res, next) => {
  try {
    const { teamId, slug, taskId, sha, loopId } = req.params as { teamId: string; slug: string; taskId: string; sha: string; loopId?: string };
    ...
    await upsertTaskCommit(teamId, slug, taskId, sha, parsed.data, loopId);
