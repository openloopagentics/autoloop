# `/daloop` driver hygiene — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Teach the `/daloop` loop-driver skill to start a loop per run, transition task/phase/loop status to terminal when done, open/fix trackable bugs, and attach test-run summaries.

**Architecture:** Prose-only change to one skill file (`plugins/daloop-reporting/skills/daloop/SKILL.md`), a plugin version bump, and a sync of the curl-installer copy. The CLI verbs all already exist (SP1/v2.1) — no code, no contract change. Correctness bar: every `daloop …` command in the file is valid against `cli/daloop.mjs`, and the existing suites stay green.

**Tech Stack:** Markdown skill file; the dependency-free `daloop` CLI; `scripts/sync-daloop-cli.sh`.

**Spec:** `docs/superpowers/specs/2026-06-03-daloop-driver-hygiene-design.md`
...
### Task 1: Rewrite the `/daloop` SKILL.md with the four hygiene behaviors

**Files:**
- Overwrite: `plugins/daloop-reporting/skills/daloop/SKILL.md`
...
**Files:**
- Modify: `plugins/daloop-reporting/.claude-plugin/plugin.json` (version `0.2.1` → `0.3.0`)
- Synced (generated): `web/public/skill/daloop/SKILL.md`

- [ ] **Step 1: Bump the plugin version**

In `plugins/daloop-reporting/.claude-plugin/plugin.json`, change `"version": "0.2.1"` to `"version": "0.3.0"` (a meaningful driver-behavior change so installed plugins pick it up).

- [ ] **Step 2: Sync the curl-installer copy**

Run: `bash scripts/sync-daloop-cli.sh`
Expected: prints the `✓ synced …` lines (it copies `plugins/daloop-reporting/skills/daloop/SKILL.md` → `web/public/skill/daloop/SKILL.md`, among the CLI/vision copies).

- [ ] **Step 3: Verify the skill copy is identical**

Run: `diff plugins/daloop-reporting/skills/daloop/SKILL.md web/public/skill/daloop/SKILL.md && echo IDENTICAL`
...
```bash
git add plugins/daloop-reporting/.claude-plugin/plugin.json web/public/skill/daloop/SKILL.md
git commit -m "chore(plugin): bump daloop-reporting 0.3.0 + sync /daloop skill copy
...
- `/daloop` SKILL.md instructs the driver to: start a loop per run (date-stamped id), set
...
- Every `daloop` command in the file is valid against `cli/daloop.mjs`.
- Plugin bumped to 0.3.0; `web/public/skill/daloop/SKILL.md` is identical to the plugin copy.
