# Daloop REST API Implementation Plan
...
functions/
  ...
  tsconfig.json
  vitest.config.ts
  .env.example                # documents DALOOP_WRITE_KEYS
  src/
...
DALOOP_WRITE_KEYS=replace-with-a-strong-key
```
...
Reads `DALOOP_WRITE_KEYS` (comma-separated) from the environment. `Authorization: Bearer <key>` is canonical; `x-api-key` is the fallback when `Authorization` is absent. Comparison is constant-time and checks every configured key (no early exit).
...
`curl -s -X DELETE "http://127.0.0.1:8080/emulator/v1/projects/daloop-test/databases/(default)/documents"`
→ expect `{}` and HTTP 200.
...
- Auth: `Authorization: Bearer <key>`; keys configured via `DALOOP_WRITE_KEYS`. Locally use `functions/.env`; in production set the secret:
