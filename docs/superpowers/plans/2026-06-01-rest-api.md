...
- [ ] **Step 2: Run to verify it fails** — `npm test -- status`
...
- [ ] **Step 4: Run to verify it passes** — `npm test -- status` → PASS
...
- [ ] **Step 2: Run to verify it fails** — `npm test -- schemas` → FAIL
...
// URL-/Firestore-safe IDs (slug, phaseId, sha). No spaces or special chars beyond . _ - /
...
// All content fields optional; server-owned fields are stripped via .strip() default + omitting them.
// We use .passthrough()-free objects so unknown keys are dropped.
export const projectBody = z
  ...
  .strict()
  .partial()
  // .strict() would reject server-owned fields; instead we want to silently ignore them:
  .or(z.object({}));
...
> **Implementer note:** the `projectBody` definition above is intentionally a starting point. The goal expressed by the test is: accept the three content fields, reject bad `status` and oversized `design.content`, and **drop** server-owned keys (`currentPhaseId`, `createdAt`, etc.) rather than erroring. The simplest correct implementation is a plain `z.object({ title, status, design }).partial()` — Zod drops unknown keys by default (no `.strict()`). Replace the `.strict().partial().or(...)` experiment with that plain form and confirm the "strips server-owned fields" test passes.

- [ ] **Step 4: Run to verify it passes** — `npm test -- schemas` → PASS (adjust the `projectBody` definition per the note until green)
...
- [ ] **Step 2: Run to verify it fails** — `npm test -- errors` → FAIL
...
- [ ] **Step 4: Run to verify it passes** — `npm test -- errors` → PASS
...
- [ ] **Step 2: Run to verify it fails** — `npm test -- auth` → FAIL
...
- [ ] **Step 4: Run to verify it passes** — `npm test -- auth` → PASS
...
Run (in a second terminal): `cd functions && npm run emulators`
Then: `curl -s -X DELETE "http://127.0.0.1:8080/emulator/v1/projects/daloop-test/databases/(default)/documents"`
Expected: `{}` (empty JSON) and HTTP 200.

> **Note for executor:** all emulator-backed tests (Tasks 6–10) require `npm run emulators` running in a separate terminal. Document this in the README in Task 11.
...
describe("PUT /v1/projects/:slug", () => {
  it("rejects unauthenticated writes", async () => {
    const res = await request(app).put("/v1/projects/acme%2Fweb").send({ title: "X", status: "queued" });
...
- [ ] **Step 2: Run to verify it fails** — `npm test -- projects` → FAIL (no `makeApp`)
...
- [ ] **Step 6: Run to verify it passes** (emulator running) — `npm test -- projects` → PASS
...
- [ ] **Step 2: Run to verify it fails** — `npm test -- phases` → FAIL
...
- [ ] **Step 6: Run to verify it passes** — `npm test -- phases` → PASS
...
- [ ] **Step 2: Run to verify it fails** — `npm test -- commits` → FAIL
...
- [ ] **Step 6: Run to verify it passes** — `npm test -- commits` → PASS
...
describe("full loop reporting flow", () => {
  it("project -> phase -> commit, with derived current phase", async () => {
    await request(app).put("/v1/projects/acme%2Fweb".replace("%2F", "/"))
...
> Note: the first line uses slug `acme` (fix the `%2F` experiment — just send `/v1/projects/acme`). Keep slugs simple in this test.

- [ ] **Step 4: Run the full suite** — `npm test` (emulator running) → all PASS
...
- Modify: `functions/vitest.config.ts` (none needed) and `functions/package.json` (add a `test:rules` script)
...
- [ ] **Step 2: Add a `test:rules` script to `functions/package.json`**
...
```bash
git add firestore.rules functions/package.json functions/test-rules/rules.test.ts
...
- Test: start emulator (`npm run emulators`) in one terminal, then `npm test` in another; rules tests run via `npm run test:rules` (self-launches the emulator).
...
- `npm test` (with the Firestore emulator running) passes all suites.
