...
> **Workflow note:** the `test` script self-launches the Firestore emulator via
> `firebase emulators:exec`, so `npm test` needs no second terminal (it does
> require Java). Use `npm run test:watch` for the fast dev loop — that bare
> `vitest` form expects `npm run emulators` running separately.
...
- [ ] **Step 4: Run to verify it passes** — `npm test -- schemas` → PASS
...
> emulator themselves. Only `npm run test:watch` needs `npm run emulators`
> running separately (the fast dev loop).
